...
func (log *Logger) Debug(ctx context.Context, msg string, args ...any) {
	log.write(ctx, LevelDebug, 3, msg, args...) //nolint:mnd
}
...
func (log *Logger) Info(ctx context.Context, msg string, args ...any) {
	log.write(ctx, LevelInfo, 3, msg, args...) //nolint:mnd
}
...
func (log *Logger) Warn(ctx context.Context, msg string, args ...any) {
	log.write(ctx, LevelWarn, 3, msg, args...) //nolint:mnd
}
...
func (log *Logger) Error(ctx context.Context, msg string, args ...any) {
	log.write(ctx, LevelError, 3, msg, args...) //nolint:mnd
}
...
func (log *Logger) write(ctx context.Context, level Level, caller int, msg string, args ...any) {
	...
	r := slog.NewRecord(time.Now(), slogLevel, msg, pcs[0]) //nolint:varnamelen
...
func newLogger(
	...
	minLevel Level,
	serviceName string,
	traceIDFn TraceIDFn, events Events,
) *Logger { //nolint:revive,varnamelen
	// Convert the file name to just the name.ext when this key/value will
	// be logged.
	f := func(groups []string, a slog.Attr) slog.Attr { //nolint:revive
	...
	handler := slog.Handler(slog.NewJSONHandler(
		w, &slog.HandlerOptions{AddSource: true, Level: slog.Level(minLevel), ReplaceAttr: f},
	))
